...
@section('content')
<div class="max-w-xl">
    <form action="{{ route('admin.sectors.update', $sector) }}" method="post" class="space-y-4">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug', $sector->slug) }}"
                   class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-[#C41E3A]/30 focus:border-[#C41E3A]">
            @error('slug')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="sort_order" class="block text-sm font-medium text-gray-700 mb-1">{{ __('admin.sort_order') }}</label>
            <input type="number" name="sort_order" id="sort_order" min="0" value="{{ old('sort_order', $sector->sort_order) }}"
                   class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-[#C41E3A]/30 focus:border-[#C41E3A]">
        </div>
